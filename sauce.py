...
def report_issued_books():
    ...
    print('-'*120)
    sql = 'select * from books where status = "issue";'
    cursor.execute(sql)
    records = cursor.fetchall()
    for record in records:
       print(record)
...
def report_weed_out_books():
    ...
    print('-'*120)
    sql = 'select * from books where status = "weed-out";'
    cursor.execute(sql)
    records = cursor.fetchall()
    for record in records:
       print(record)
...
def report_stolen_books():
    ...
    print('-'*120)
    sql = 'select * from books where status = "stolen";'
    cursor.execute(sql)
    records = cursor.fetchall()
    for record in records:
       print(record)
...
def report_fine_collection():
    conn = mysql.connector.connect(
        host='localhost', database='library', user='root', password='1234')
    cursor = conn.cursor()
    sql ='select sum(fine) from transactions where dor ="'+str(date.today())+'";'
    cursor.execute(sql)
...
def change_book_status(status,book_id):
    conn = mysql.connector.connect(
        host='localhost', database='library', user='root', password='1234')
    cursor = conn.cursor()
    sql = 'update books set status = "'+status +'" where id ='+book_id + ' and status ="available"'
    cursor.execute(sql)
